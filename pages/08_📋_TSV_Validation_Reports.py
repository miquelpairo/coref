...
def filter_relevant_data(data: List[Dict]) -> List[Dict]:
    ...
    pixel_cols = sorted([c for c in all_columns if _is_pixel_col(c)], key=lambda s: int(str(s)[1:]))
...
def clean_tsv_file(uploaded_file) -> pd.DataFrame:
    ...
    for encoding in encodings_to_try:
        try:
            ...
            try:
                uploaded_file.seek(0)
                df_raw = pd.read_csv(uploaded_file, delimiter="\t", keep_default_na=False, encoding=encoding, doublequote=False, escapechar="\\")
                break
            except Exception:
                continue
    if df_raw is None:
    ...
    if st.button("🔄 Procesar Archivos con Filtros", type="primary", use_container_width=True):
        ...
        for idx, uploaded_file in enumerate(uploaded_files, start=1):
            ...
            try:
                ...
                st.session_state.last_event_id[file_name] = ""
...
if st.session_state.processed_data:
    ...
    if selected_file:
        ...
        st.session_state.last_event_id.setdefault(selected_file, "")
        ...
        for group_key in group_keys:
            with st.expander(f"{SAMPLE_GROUPS[group_key]['emoji']} {group_key}", expanded=False):
                ...
                with c1:
                    new_label = st.text_input("Etiqueta:", value=st.session_state.group_labels[group_key], key=f"label_{selected_file}_{group_key}", max_chars=30)
                    if new_label != st.session_state.group_labels[group_key]:
                        st.session_state.group_labels[group_key] = new_label
                with c2:
                    new_desc = st.text_area("Descripción:", value=st.session_state.group_descriptions[group_key], key=f"desc_{selected_file}_{group_key}", max_chars=200, height=100)
        ...
        if not columns_result:
            ...
            try:
                plots = plot_comparison_preview(df_current, result_col, reference_col, residuum_col, removed_indices, sample_groups, st.session_state.group_labels, SAMPLE_GROUPS)
                if not plots:
        ...
        with st.expander("📋 Tabla de Muestras", expanded=False):
            edited_df = st.data_editor(
                ...
                column_config={
                    ...
                    "Grupo": st.column_config.SelectboxColumn("Grupo", options=["none", "Set 1", "Set 2", "Set 3", "Set 4"], default="none")
                }, 
        ...
        with c2:
            if st.button("🗑️ Confirmar Eliminación", use_container_width=True, disabled=(len(removed_indices) == 0)):
                if removed_indices:
                    ...
                    st.rerun()
        with c3:
            if st.button("↩️ Limpiar Todo", use_container_width=True, disabled=(len(removed_indices) == 0 and len(sample_groups) == 0)):
                st.session_state.samples_to_remove[selected_file] = set()
...
if st.session_state.processed_data:
    ...
    for fname, df in st.session_state.processed_data.items():
        ...
        summary_data.append({"Archivo": fname, "Muestras": len(df), "Agrupadas": grouped_count, "Parámetros": len([c for c in df.columns if str(c).startswith("Result ")])})
    ...
    if st.button("📥 Generar Informes HTML", type="primary", use_container_width=True):
        ...
        for idx, (file_name, df) in enumerate(st.session_state.processed_data.items(), start=1):
            try:
                ...
                html = generate_html_report(df, file_name, sample_groups_file, st.session_state.group_labels, st.session_state.group_descriptions, SAMPLE_GROUPS, PIXEL_RE)
        ...
        if results:
            ...
            if len(results) > 1:
                ...
                st.download_button("📦 Descargar ZIP", data=zip_buffer.getvalue(), file_name="reports.zip", mime="application/zip", use_container_width=True)
                st.markdown("---")
            for r in results:
                st.markdown(f"**{r.name}**")
                st.download_button("💾 Descargar HTML", data=r.html, file_name=f"{r.name}.html", mime="text/html", key=f"dl_{r.name}")
                st.markdown("---")
