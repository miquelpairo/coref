...
def render_reference_tsv_section():
    ...
    if has_reference_tsv():
        ...
        # ⭐ Mostrar solo las filas seleccionadas en el Paso 3
        if 'selected_wstd_indices' in st.session_state and st.session_state.selected_wstd_indices:
            ...
            st.success(f"✅ TSV de referencia del Paso 3: {len(df_ref)} filas seleccionadas")
        else:
            df_ref = df_ref_full.copy()
            st.warning(f"⚠️ TSV de referencia cargado ({len(df_ref)} filas) - No hay selección del Paso 3")
        
        # Mostrar preview SOLO de filas seleccionadas
        with st.expander("🔎 Ver filas seleccionadas del TSV de referencia", expanded=False):
            if 'selected_wstd_indices' in st.session_state and st.session_state.selected_wstd_indices:
                # Mostrar con índices originales
                display_df = df_ref[['ID', 'Note']].copy()
                display_df.insert(0, 'Índice original', st.session_state.selected_wstd_indices)
                st.dataframe(display_df, use_container_width=True)
                st.info(f"📋 Mostrando las {len(df_ref)} filas que seleccionaste en el Paso 3")
            else:
        ...
        if st.checkbox("📂 Cargar otro archivo TSV de referencia", key="override_ref_tsv"):
            ...
            if new_ref_file:
                try:
                    ...
                    st.info("✅ Marca las casillas de las mediciones que quieres usar.")
                    
                    # Crear tabla con índice visible
                    df_display = df_ref_new[['ID', 'Note']].copy()
                    ...
                    edited_df = st.data_editor(
                        ...
                        disabled=['ID', 'Note'],
                        key='override_ref_row_selector'
                    )
                    
                    selected_indices = edited_df[edited_df['Seleccionar'] == True].index.tolist()
                    
                    if len(selected_indices) > 0:
                        if st.button("✅ Confirmar selección y reemplazar", type="primary"):
                            ...
                            st.success(f"✅ {len(selected_indices)} filas seleccionadas y guardadas")
                            st.rerun()
                    else:
...
def render_new_tsv_section():
    ...
    if new_file:
        ...
        try:
            ...
            # ⭐ NUEVO: Selección de filas
            st.markdown("#### Selecciona las filas a usar")
            st.info("✅ Marca las casillas de las mediciones que quieres usar para el alineamiento.")
            
            # Crear tabla con índice visible
            df_display = df_new[['ID', 'Note']].copy()
            df_display.insert(0, 'Seleccionar', False)
            
            if 'new_tsv_selected_rows' not in st.session_state:
                st.session_state.new_tsv_selected_rows = []
            ...
            edited_df = st.data_editor(
                ...
                use_container_width=True,
                disabled=['ID', 'Note'],
                key='new_tsv_row_selector'
            )
            
            selected_indices = edited_df[edited_df['Seleccionar'] == True].index.tolist()
            
            if len(selected_indices) == 0:
                st.warning("⚠️ No has seleccionado ninguna fila. Por favor, marca las mediciones a usar.")
                return False
            else:
                ...
                st.session_state.new_tsv_selected_indices = selected_indices
                
                # Mostrar info detallada
                st.write("**Filas seleccionadas:**")
                display_df = df_new_selected[['ID', 'Note']].copy()
                display_df.insert(0, 'Índice fila', selected_indices)
                st.dataframe(display_df, use_container_width=True)
                
                # Guardar en session_state temporal con SOLO las filas seleccionadas
                st.session_state.new_tsv = {
                    'df': df_new_selected,  # ← Solo filas seleccionadas
    ...
    if 'new_tsv' in st.session_state:
        ...
        if 'new_tsv_selected_indices' in st.session_state:
            st.info(f"📋 {len(st.session_state.new_tsv_selected_indices)} filas seleccionadas")
        return True
...
def render_sample_selection_section():
    ...
    # ⭐NUEVO: Filtrar solo las filas seleccionadas en Paso 3
    if 'selected_wstd_indices' in st.session_state and st.session_state.selected_wstd_indices:
        df_ref = df_ref_full.loc[st.session_state.selected_wstd_indices].copy()
        st.info(f"📋 Usando {len(df_ref)} filas seleccionadas del Paso 3")
    else:
        df_ref = df_ref_full.copy()
        st.warning("⚠️ No hay selección del Paso 3. Usando todas las filas.")
