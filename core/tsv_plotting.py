    ...
    try:
        ...
        for group_name, group_config in SAMPLE_GROUPS.items():
            ...
            if indices:
                ...
                hovertext_group = [
                    f"{display_label}<br>Index: {original_indices[i]}<br>Date: {hover_date.iloc[i]}<br>ID: {hover_id.iloc[i]}<br>Reference: {x.iloc[i]:.2f}<br>Result: {y.iloc[i]:.2f}"
                    for i in indices
                ]
        ...
        if indices:
            ...
            y_group = y.iloc[indices]
            hovertext_group = [
                f"Sin grupo<br>Index: {original_indices[i]}<br>Date: {hover_date.iloc[i]}<br>ID: {hover_id.iloc[i]}<br>Reference: {x.iloc[i]:.2f}<br>Result: {y.iloc[i]:.2f}"
                for i in indices
            ]
            fig_parity.add_trace(go.Scatter(
                x=x_group,
        ...
        if indices_delete:
            ...
            hovertext_remove = [
                f"⚠️ MARCADO PARA ELIMINAR<br>Index: {original_indices[i]}<br>Date: {hover_date.iloc[i]}<br>ID: {hover_id.iloc[i]}<br>Reference: {x.iloc[i]:.2f}<br>Result: {y.iloc[i]:.2f}"
        ...
        hovertext_res = [
            f"Sin grupo<br>Date: {date_val}<br>ID: {id_val}<br>Residuum: {res_val:.2f}"
